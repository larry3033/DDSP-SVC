...
def parse_args(args=None, namespace=None):
    ...
    parser.add_argument("-c", "--config", type=str, default=r'configs/reflow.yaml')
...
def preprocess(id, encoder, encoder_ckpt, encoder_sample_rate, encoder_hop_size,cnhubertsoft_gate, vocoder_type, vocoder_ckpt, f0_min, f0_max,f0_extractor_type, use_pitch_aug, pitch_aug_dict, filelist, num_processes, path,  sample_rate, hop_size, device = 'cuda', extensions = ['wav']):
    ...
    units_encoder = Units_Encoder(encoder, encoder_ckpt, encoder_sample_rate, encoder_hop_size,cnhubertsoft_gate=cnhubertsoft_gate,device = device)    
...
if __name__ == '__main__':
    ...
    mp.spawn(preprocess, args=(encoder, encoder_ckpt, encoder_sample_rate, encoder_hop_size,cnhubertsoft_gate, vocoder_type, vocoder_ckpt, f0_min, f0_max,f0_extractor_type, use_pitch_aug, pitch_aug_dict, filelist, num_processes, path,  sample_rate, hop_size), nprocs=num_processes)
